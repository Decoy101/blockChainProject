...
contract BestX is Storage {
    ...
    function createPost (string memory _tokenUrl, string memory _description) public payable nonReentrant returns (bool _success) {
    ...
    function postsByUser (address _artist) public view returns (Post[] memory) {
        ...
        for (uint i = 0; i < totalItemCount; i++) {
            if (itemIdToPost[i].artist == _artist) {
                itemCount += 1;
            }
        }
    ...
    function _calculateTop5Payout() public onlyOwner returns (bool _success) {
        ...
        uint _pool = _difference.mul(postingPrice);
        uint _fee = _calculateFeeAmount(_pool);
        jackpot = _pool.sub(_fee);
